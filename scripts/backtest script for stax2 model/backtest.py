#!/usr/bin/env python3
"""
Stax Meta-Model - FLAT STAKE BACKTESTER
This script loads all pre-trained base and meta models and runs the 
advanced backtester to evaluate performance using a flat £10 stake per bet.
"""
...
class ValueBettingBacktester:
    """Advanced backtester that uses a FLAT STAKE for all bets."""
    ...
    def should_place_bet(self, probs: np.ndarray, odds: List[float], 
        ...
        Returns a fixed stake of STAKE_PER_BET if conditions are met.
        """
    ...
    def run_advanced_backtest(self, backtest_dir: Path, strategies: list, 
        ...
        print(f"\nRunning FLAT STAKE backtest on {len(json_files)} matches...")
            ...
            for value_threshold in value_thresholds:
                ...
                for file_path in tqdm(json_files, desc="Backtesting", leave=False):
                    ...
                    for strategy_minutes in strategies:
                        ...
                        if time_s > (df_xgb['time_elapsed_s'].max()): continue

                        target_idx = (df_xgb['time_elapsed_s'] - time_s).abs().idxmin()
                        if target_idx < SEQUENCE_LENGTH: continue
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Stax Meta-Model Flat Stake Backtester')
    ...
    print("=== Starting Flat Stake Backtest Pipeline ===")
    ...
    print("\n--- Stax Flat Stake Backtest Complete ---")
